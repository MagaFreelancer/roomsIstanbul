...
const SinglePage: FC = (): JSX.Element => {
    ...
    const extendDate = arentedDate ? addDays(new Date(arentedDate.rentedDate), arentedDate.daysCount) : new Date()
    const minDate = arentedDate ? arentedDate.daysCount + 1 : 0
    const addedStartDateDays: number = arentedDate ? 2 : 0;
    const addedEndDateDays: number = arentedDate ? 3 : 1;


    const [valueDateRangePicker, setValueDateRangePicker] = useState<Range[] | undefined>([
        {
            startDate: addDays(extendDate, addedStartDateDays),
            endDate: addDays(extendDate, addedEndDateDays),
            key: "selection",
        },
    ]);
    ...
    const onClickRent = async () => {
        ...
        const arentedDate = changedData.rentedRooms.find(item => item.id === singleRoom.id)
        const extendDate = arentedDate ? addDays(new Date(), arentedDate.daysCount) : new Date()

        setValueDateRangePicker(
            [
                {
                    startDate: addDays(extendDate, 1),
                    endDate: addDays(extendDate, addedEndDateDays),
                    key: "selection",
                }
            ]
                                        ...
                                        <div className="singlepage__comments-heading">
                                            ...
                                            <div className="singlepage__comments-info">
                                                ...
                                                <div className="singlepage__comments-rating">
                                                    {item.rating && <Rating emptyIcon={<StarIcon style={{ opacity: 0.45 }} fontSize="inherit" />} name="hover-feedback" size="small" defaultValue={item.rating} precision={0.25} readOnly />}
                                                </div>
                                            </div>
                                        </div>
